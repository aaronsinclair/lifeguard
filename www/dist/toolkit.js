...
var App = {
  ...
  _wizardProgress: function _wizardProgress() {

    // Form Wizard
    $(".wizard").each(function (i, el) {
      var $this = $(el),
          $progress = $this.find(".steps-progress div"),
          $nav = $this.find('.wizard-steps > ul'),
          $items = $nav.children().width(100 / $nav.children().length + '%'),
          $tab = $nav.find('> li a.active').parent();

      setCurrentProgressTab($this, $nav, $tab, $progress);

      $('a[data-toggle="tab"]', $this).on('shown.bs.tab', function (e) {
        var $tab = $this.find('[href="' + $(e.target).attr('href') + '"]').closest('li');
        setCurrentProgressTab($this, $nav, $tab, $progress);
      });
    });

    // Root Wizard Current Tab
    function setCurrentProgressTab($wizard, $nav, $tab, $progress) {
      $('a[data-toggle="tab"]', $wizard).removeClass('active completed');
      $('a', $tab).addClass('active');
      $tab.prevAll().find('a').addClass('completed');

      var index = $tab.index(),
          items = $nav.children().length,
          pct = parseInt((index + 1) / items * 100, 10),
          first_tab = $tab.is(':first-child'),
